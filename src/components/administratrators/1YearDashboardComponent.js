...
export class OneYearDashboardComponent extends Component {
  ...
  exportToPdf = () => {
    const input = document.getElementById('a1yearDashboard');
    html2canvas(input).then(canvas => {
      ...
      doc.save('1yearDashboard.pdf');
    });
  };

  onChange = e => {
    this.setState({ [e.target.name]: e.target.value });
    this.props.fetchPeriodicScoreboards('1year', e.target.value);
  };

  componentDidMount() {
    this.props.fetchPeriodicScoreboards('1year', this.state.departmentId);
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        <div className="my-3" id="a1yearDashboard">
