...
export class ScoreboardsListComponent extends Component {
  ...
  render() {
    ...
    if (scoreboards.length === 0 && !isLoading) {
      return (
        <div>
          {' '}
        ...
        return (
          <React.Fragment key={scoreboard.id}>
            <tr>
              ...
              <td className="">
                {/* The approve value should be coming from the database */}
                <p>
                  <button type="button" className="btn btn-light">
                    {scoreboard.isApproved ? 'Approved' : 'Not Approved'}
                  </button>
                </p>
                <p style={{ display: scoreboard.isApproved ? 'none' : '' }}>
                  <button
                    type="button"
                    onClick={() =>
                      this.props.history.push(
                        `/employee/edit-scores/${scoreboard.id}`
                      )
                    }
