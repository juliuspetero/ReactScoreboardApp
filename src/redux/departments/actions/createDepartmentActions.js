...
import validateCreateDepartment from '../../../helpers/validateCreateDepartment';
...
export const createDepartment = department => {
  ...
  return dispatch => {
    ...
    const { errors, isValid } = validateCreateDepartment(department);
    // console.log(kpi);
    // return;
    if (!isValid) dispatch(createKPIFailure({ data: errors }));
    else {
      axios
        .post(`${config.baseUrl}/departments`, department)
        .then(response => {
